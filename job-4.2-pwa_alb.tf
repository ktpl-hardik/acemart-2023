resource "aws_security_group" "pwa_alb_sg" {
  name        = "${var.acemart_prod_owner}-pwa-alb-sg"
  description = "Allow HTTP traffic to pwa instances through pwa Application Load Balancer."
...
# Create a basic ALB 
resource "aws_lb" "pwa_alb" {
  name               = "${var.acemart_prod_owner}-pwa-alb"
  internal           = false
...
resource "aws_lb_target_group" "pwa_target" {
  name = "${var.acemart_prod_owner}-pwa-target"
  port = 80
  protocol = "HTTP"
  vpc_id = aws_vpc.vpc.id
...
resource "aws_lb_listener" "pwa_alb_listener" {
  default_action {
    target_group_arn  = "${aws_lb_target_group.pwa_target.arn}"
    type              = "forward"
  }
  load_balancer_arn   = "${aws_lb.pwa_alb.arn}"
  port                = "80"
  protocol            = "HTTP"
}

# Create a Listener - HTTPS
resource "aws_lb_listener" "pwa_alb_listener_https" {
  default_action {
    target_group_arn  = "${aws_lb_target_group.pwa_target.arn}"
    type              = "forward"
  }
...
#resource "aws_lb_listener_rule" "pwa_rule_1" {
#  action {
#    target_group_arn = "${aws_lb_target_group.pwa_target.arn}"
#    type = "forward"
#  }
...
#  }
#
#  listener_arn = "${aws_lb_listener.pwa_alb_listener.id}"
#  priority = 100
#}

#Note:
#pwa ALB SG add to the job-2-security_group.tf